...
from eagle.mm_utils import tokenizer_image_token, tokenizer_image_token_llama3 ## ADDED SUPPORT FOR LLAMA3

from PIL import Image

from datasets import load_from_disk ### ADDED SUPPORT FOR NON-JSON DATA FILES
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    sequential: bool = field(default=False) ### ADDED SUPPORT FOR SEQUENTIALSAMPLER
...
def preprocess(
    ...
    if conversation_lib.default_conversation.sep_style == conversation_lib.SeparatorStyle.LLAMA_3: ### (COPIED FROM CAMBRIAN CODEBASE)
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        ### ADDED SUPPORT FOR NON-JSON DATA FILES
    ...
    def modality_lengths(self):
        length_list = []
        for sample in self.list_data_dict:
            ### ADDED SUPPORT FOR TEXT_ONLY TRAINING
            if "text_only" in sample and sample["text_only"]:
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        ### ADDED SUPPORT FOR TEXT_ONLY TRAINING
        if "text_only" in sources[0] and sources[0]["text_only"]:
            input_ids = torch.tensor(sources[0]["input_ids"])
            labels = copy.deepcopy(input_ids)
            crop_size = self.data_args.image_processor.crop_size
            image = torch.zeros(3, crop_size['height'], crop_size['width'])
            data_dict = dict(input_ids=input_ids, labels=labels, image=image)
            return data_dict
        ...
        ### ADDED SUPPORT FOR TEXT_ONLY TRAINING
        has_image = 'image' in sources[0] or ('image_RGB' in sources[0] and len(sources[0]['image_RGB']) > 0) ## HACKY (ONLY BECAUSE TEXT ONLY INPUT HAS 'image_RGB' = '')
        if has_image:
            if 'image' in sources[0]:
                image_file = self.list_data_dict[i]['image']
                image_folder = self.data_args.image_folder
                ...
                image_file, image_index = sources[0]['image_RGB']
                image_RGB = np.load(image_file)[image_index]
                processor = self.data_args.image_processor
        ...
        data_dict = preprocess(
            sources,
            self.tokenizer,
            has_image=has_image) ### ADDED SUPPORT FOR TEXT_ONLY TRAINING
        ...
        if has_image: ### ADDED SUPPORT FOR TEXT_ONLY TRAINING
...
def train(attn_implementation=None):
    ...
    elif model_args.version == "llava_llama_3": ### (COPIED AND ADAPTED FROM CAMBRIAN CODEBASE)
    ...
    if not (list(pathlib.Path(training_args.output_dir).glob("model-*.safetensors"))):
