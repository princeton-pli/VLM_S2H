...
def mix_data(data_list):
    for data in data_list:
        np.random.shuffle(data)

    gcd = math.gcd(*[len(data) for data in data_list])
    num_data = [len(data) // gcd for data in data_list]
    chunks = [[data[i:i+num] for i in range(0, len(data), num)] for num, data in zip(num_data, data_list)]
    
    # interleave the chunks
    merged = []
    for chunk_group in zip(*chunks):  
...
## { "SIMPLE": [I->T, T->S, I->S, I->T+S], "MEDIUM": [I->T, T->S, I->S, I->T+S] }
def get_data_composition(option):

    # Train only on SIMPLE examples
    ## EPOCHS = 3
    if option == 1: # Image-via-text supervision
        return {"SIMPLE": [False, False, False,  True], "MEDIUM": [False, False, False, False]}
    
    ## EPOCHS = 2
    if option == 2: # Text supervision
        return {"SIMPLE": [False,  True, False, False], "MEDIUM": [False, False, False, False]}
    
    ## EPOCHS = 2
    if option == 3: # Image supervision with epochs 2
        return {"SIMPLE": [False, False,  True, False], "MEDIUM": [False, False, False, False]}
    ...
    if option == 5: # Image+Text supervision with epochs 1.5
        return {"SIMPLE": [False,  True,  True, False], "MEDIUM": [False, False, False, False]}

    ## EPOCHS = 1
    if option == 6: # Mix supervision with epochs 1
        return {"SIMPLE": [False,  True,  True,  True], "MEDIUM": [False, False, False, False]}

    # Train on SIMPLE+MEDIUM examples
    if option == 7: # Mix+ supervision with epochs 1
        return {"SIMPLE": [False,  True,  True,  True], "MEDIUM": [False,  True, False, False]}
 
    if option == 8: # First phase of supervision for Align-Mix+ 
        return {"SIMPLE": [False,  True, False,  True], "MEDIUM": [False, False, False, False]}

    if option == 10: # Mix+ supervision with epochs 1
        return {"SIMPLE": [False,  True,  True,  True], "MEDIUM": [False,  True, False, False]}
...
def main(args):
    ...
    out_base_dir = os.path.join("scripts_qwen", base_dir)
    ...
    num_data_dict = {
        "SIMPLE": total_num_data // (any(data_composition["SIMPLE"]) + any(data_composition["MEDIUM"])) * any(data_composition["SIMPLE"]),
        "MEDIUM": total_num_data // (any(data_composition["SIMPLE"]) + any(data_composition["MEDIUM"])) * any(data_composition["MEDIUM"]),
    }
    ...
    if args.option in [9]:
        for domain in ["SIMPLE"]:
            ...
            for data_type, idx in data_type_map.items():
                ...
                if not args.get_data_path_only:
                    result.append(get_data(base_dir, data_type, cot_tag, domain, num_data_ // 3))
        ...
        for domain in ["MEDIUM"]:
            ...
            for data_type, idx in data_type_map.items():
                ...
                if not args.get_data_path_only:
                    result.append(get_data(base_dir, data_type, cot_tag, domain, num_data_))
    else:
        for domain in ["SIMPLE", "MEDIUM"]:
            num_data = num_data_dict[domain]
            print("------------------------------")
            print("{}: {}".format(domain, num_data))
            if num_data <= 0: continue
    ...
    if not args.get_data_path_only:
        print (len(result))
        os.makedirs(os.path.join(out_base_dir, "json_files"), exist_ok=True)
        with open(os.path.join(out_base_dir, "json_files", out_filename), "w") as f:
            json.dump(result, f, indent=2)
            print("Saved data to:", os.path.join(out_base_dir, "json_files", out_filename))
    print("------------------------------")
    print(os.path.join(out_base_dir, "json_files", out_filename))
